...
% Note 2023/08/02
% Attinger added a minimum flag so that the text output of the function reports the minimum value of the data for each set of data

function [theText,rawN, x] = nhist_Cocalc(cellValues, varargin)
%% INITIALIZE PARAMETERS
% Default initialization of the parameters,
debug_on_error(0);

%test if code is run on Matlab or Octavetest_interpreter=["Is the interpreter Matlab or " "Octave?"];
...
elseif (size(test_interpreter) == [1 2])
  ...
  warning('It looks like the interpreter is neither Matlab nor Octave')
end
...
boxplotFlag = 0;
textFlag=0;
minimumFlag=1;% added by Attinger, 2023/8/2
decimalPlaces=2;
legendExists=0;
linewidth=2;
newfigFlag=0;
...
%     figure('Name', Title);
    ...
    hold_state = ishold;
    if ~hold_state %create figure here
...
for k=1:num2Plot
    ...
    if isData(k)
        if textFlag
            ...
            if minimumFlag
                theText{k}=[theText{k} 'minimum='  num2str(minimumV{k},['%.' num2str(decimalPlaces) 'f']) ', '];
            end
            if modeFlag
                theText{k}=[theText{k} 'approx mode='  num2str(roundedMode{k},['%.' num2str(decimalPlaces) 'f']) ', '];
            end
